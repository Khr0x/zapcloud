...
use zc_persistence::{
    Database, NewArtifact, NewFunction, PersistenceError, UpdateCodeResult,
    UpdateCodeWithArtifactResult,
};
...
async fn artifact_dedup_por_sha256() {
    ...
    assert_eq!(
        a.id, b.id,
        "el segundo put debe reusar el artifact existente"
    );
...
async fn function_crud_completo() {
    ...
    assert_eq!(
        created.latest_artifact_id.as_deref(),
        Some(artifact.id.as_str())
    );

    // Get
    let got = db
        .get_function_by_name("invoice-worker")
        .await
        .unwrap()
        .unwrap();
    ...
    db.create_function(sample_function("aaa-first", None))
        .await
        .unwrap();
    ...
    let page = db
        .list_function_artifacts_page(None, 1)
        .await
        .expect("primera página");
    assert_eq!(page.len(), 1);
    assert_eq!(page[0].function.name, "aaa-first");
    assert!(page[0].artifact.is_none());
    let page = db
        .list_function_artifacts_page(Some("aaa-first"), 1)
        .await
        .expect("segunda página");
    assert_eq!(page[0].function.name, "invoice-worker");
    assert_eq!(page[0].artifact.as_ref().unwrap().id, artifact.id);
    ...
    assert_eq!(
        updated.latest_artifact_id.as_deref(),
        Some(artifact2.id.as_str())
    );
    assert_ne!(
    ...
    assert!(db
        ...
        .is_none());
    assert!(
        !db.delete_function("invoice-worker").await.unwrap(),
        "borrar de nuevo = false"
    );
}
...
async fn nombre_de_funcion_es_unico() {
    let db = setup().await;
    db.create_function(sample_function("dup", None))
        .await
        .expect("create #1");
...
#[tokio::test]
async fn create_con_nombre_duplicado_devuelve_conflicto_tipado() {
    let db = setup().await;
    let artifact = NewArtifact {
        sha256: "create-conflict".to_string(),
        size: 1,
        media_type: "application/zip".to_string(),
        storage_path: "/artifacts/create-conflict".to_string(),
    };
    db.create_function_with_artifact(artifact.clone(), sample_function("racing", None))
        .await
        .expect("create #1");
    let error = db
        .create_function_with_artifact(artifact, sample_function("racing", None))
        .await
        .expect_err("create #2 debe fallar");
    assert!(matches!(
        error,
        PersistenceError::FunctionNameConflict(name) if name == "racing"
    ));
}

#[tokio::test]
async fn update_atomico_revierte_artifact_si_revision_es_obsoleta() {
    let db = setup().await;
    let base = db
        .put_artifact(NewArtifact {
            sha256: "update-base".to_string(),
            size: 1,
            media_type: "application/zip".to_string(),
            storage_path: "/artifacts/update-base".to_string(),
        })
        .await
        .unwrap();
    let function = db
        .create_function(sample_function("atomic", Some(base.id)))
        .await
        .unwrap();
    ...
    let result = db
        .update_function_code_with_artifact(
            NewArtifact {
                sha256: "update-rejected".to_string(),
                size: 1,
                media_type: "application/zip".to_string(),
                storage_path: "/artifacts/update-rejected".to_string(),
            },
            "atomic",
            Some("stale"),
        )
    ...
    assert_eq!(
        result,
        UpdateCodeWithArtifactResult::RevisionMismatch(function.revision_id)
    );
    assert!(db
        .get_artifact_by_sha256("update-rejected")
        .await
        .unwrap()
        .is_none());
}
